...
B<Note> : not all the CUPS options are currently implemented. If you get a message C<< unsupported option >> when
...
=item * C<< /software/components/cups/options/ServerAlias >> : list of string

This option sets the C<ServerAlias> option in cupsd configuration. It is interpreted as a list of string.
