...
type prometheus_duration =
    string with match(SELF,
        '((([0-9]+)y)?(([0-9]+)w)?(([0-9]+)d)?(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?|0)');
type prometheus_tls_client_auth = choice('NoClientCert', 'RequestClientCert', 'RequireAnyClientCert',
    'VerifyClientCertIfGiven', 'RequireAndVerifyClientCert');
type prometheus_tls_version = choice('TLS10', 'TLS11', 'TLS12', 'TLS13');
type prometheus_scheme = choice('http', 'https');
type prometheus_relabel_action = choice('replace', 'lowercase', 'uppercase', 'keep', 'drop', 'hashmod',
