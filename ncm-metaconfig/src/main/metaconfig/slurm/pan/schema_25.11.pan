declaration template metaconfig/slurm/schema_25.11;
...
type slurm_scheduler_parameters = {
    ...
    'time_min_as_soft_limit' ? boolean
    'whole_hetjob' ? boolean
};
...
type slurm_select_type_parameters = {
    ...
    'CR_Memory' ? boolean
    'ENFORCE_BINDING_GRES' ? boolean
    'LL_SHARED_GRES' ? boolean
    'MULTIPLE_SHARING_GRES_PJ' ? boolean
    'ONE_TASK_PER_SHARING_GRES' ? boolean
};
...
type slurm_mpi_params = {
    ...
    'disable_slurm_hydra_bootstrap' ? boolean
};
...
type slurm_auth_info = {
    'cred_expire' ? long(1..)
    'socket' ? absolute_file_path
    'ttl' ? long(1..)
    'use_client_ids' ? boolean
};
...
type slurm_conf_control = {
    ...
    'AuthInfo' ? slurm_auth_info
    'AuthType' ? choice('munge', 'slurm')
...
type slurm_ctld_parameters = {
    ...
    'cloud_dns' ? boolean
    'conmgr_max_connections' ? long(0..)
    'conmgr_threads' ? long
    'conmgr_use_poll' ? long
    'conmgr_connect_timeout' ? long
    'conmgr_read_timeout' ? long
    'conmgr_quiesce_timeout' ? long
    'conmgr_wait_write_delay' ? long
    'conmgr_write_timeout' ? long
    'disable_triggers' ? boolean
    'enable_configless' ? boolean
    'enable_job_state_cache' ? boolean
    'enable_stepmgr' ? boolean
    ...
    'max_powered_nodes' ? long(0..)
    'no_quick_restart' ? boolean
...
type slurm_conf_process = {
    ...
    'SwitchType' ? choice("hpe_slingshot", "nvidia_imex")   # prefix = "switch/"
    'SwitchParameters' ? string  # Todo more specific if needed
    'TaskPlugin' ? choice('affinity', 'cgroup', 'none')[]
    'TaskPluginParam' ? slurm_task_plugin_param
    'TopologyParam' ? slurm_topology_param
    'TopologyPlugin' ? choice('3d_torus', 'block', 'default', 'tree')
};
...
type slurm_conf_scheduling = {
    ...
    'SelectType' ? choice('bluegene', 'cray', 'linear', 'serial', 'cons_tres')  # prefix="select/"
...
type slurm_conf_accounting = {
    ...
    'AccountingStorageParameters' ? slurm_accounting_storage_type
    'AccountingStoragePass' ? string
    'AccountingStoragePort' ? long(0..)
    'AccountingStorageTRES' ? string[]
    'AccountingStorageType' ? choice("slurmdbd")
    'AccountingStorageUser' ? string
    'AccountingStoreFlags' ? choice('job_comment', 'job_env', 'job_script', 'job_extra', 'no_stdio')[]

    'AcctGatherNodeFreq' ? long(0..) # for acct_gather_energy/rapl plugin set a value less than 300
    'AcctGatherEnergyType' ? choice('ipmi', 'rapl', 'xcc', 'gpu', 'pm_counters')
...
type slurm_cgroups_conf = {
    ...
    'CgroupPlugin' ? choice('cgroup/v1', 'cgroup/v2', 'autodetect', 'disabled')
    ...
    'MinRAMSpace' ? long(0..)
    'SystemdTimeout' ? long(0..)
    'SignalChildrenProcesses' ? boolean
};
