...
The name of the template to be used is derived from the file name to
be rendered. It must be located under
C</usr/share/templates/quattor/metaconfig/<template_name>>.

The template name takes the name of the last directory and of the file
itself, replacing the extension with C<tt>.

For instance, C</etc/icinga/objects/services.cfg> would require
template
C</usr/share/templates/quattor/metaconfig/objects/services.tt>.
