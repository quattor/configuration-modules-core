...
##########################################################################
    ...
    security_file($self, 'allow', $cron_config->{allow}) if (exists $cron_config->{allow});
    security_file($self, 'deny', $cron_config->{deny}) if (exists $cron_config->{deny});
    ...
    for my $entry (@{$cron_entries}) {
        ...
        my $cronfh = CAF::FileWriter->open($file,
                                           mode => 0644,
                                           log => $self);
        # add header
        ...
        if ( $log_disabled ) {
            ...
            if ($syslog_method eq "logger") {
                ...
                print $cronfh "$syslog_logger_fn -t $tag -p $syslog_parms->{facility}.$syslog_parms->{level}\n";
            }
        } else {
            if (! $log_to_file) {
