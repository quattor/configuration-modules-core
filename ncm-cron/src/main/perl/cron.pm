...
##########################################################################
    ...
    if ($config->elementExists("/system/archetype")) {
        $osname = $config->getElement("/system/archetype/os")->getValue;
    }
    if (! grep { $osname eq $_ } @supportedOS) {
        $self->error("OS $osname unknown. Aborting.");
        return;
    }
        ...
        while (my $fil = readdir DIR) {  # Scan each crontab
            ...
            if (-f $fPath) {
                ...
                foreach my $line (split("\n", "$fh")) {
                    # If it is an old style ncm manged cron file ncm is
                    # the only contributor, so simply delete all content.
                    $line =~ /$ncmOldFile/ && do {
...
sub Unconfigure {
    ...
    if ($osname eq "solaris") {
        ...
        while (my $fil = readdir DIR) {  # Scan each crontab
            my $fPath = "$solaris_crondir/$fil";
            if (-f $fPath) {
                $self->info("Removing NCM from $fPath");
                my $fh = CAF::FileEditor->open($fPath, log=>$self);
                my $outlines = "";
                my $foundNCM = 0;
                foreach my $line (split("\n", "$fh")) {
                    $line =~ /$ncmOldFile/ && do {  # Old style, so stop checking
                            $outlines = "";
                            last;
                        };
                    $line =~ /$ncmStart/ && do {$foundNCM = 1};
        ...
        closedir DIR;
    } else {  # Delete Linux ncm-cron files
        opendir DIR, $linux_crondir;
        while (my $to_unlink = readdir DIR) {
            next if $to_unlink !~ /$cron_entry_regexp$/;
            $to_unlink = "$linux_crondir/$to_unlink";
            if ($to_unlink =~ m{^($linux_crondir/.*$cron_entry_regexp)$}) {
                $to_unlink = $1;  # $to_unlink is now untainted
                if (unlink($to_unlink)) {
                    $self->info("Deleting file $to_unlink");
                } else {
                    $self->error("Deleting file $to_unlink failed ($!)");
                }
            } else {
                $self->error("Bad data in $to_unlink");
            }
