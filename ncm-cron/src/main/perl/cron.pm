...
##########################################################################
  ...
  unlink $to_unlink;
    ...
    for my $entry (@{$cron_entries}) {
        ...
        # If there is no log file associated with the cron, disable execution of 'date' command to
        # avoid sending an email to root at each execution (because of date output).
