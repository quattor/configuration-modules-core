...
Tests for the NCM::Component::spma::yumng SSL configuration.
These tests verify that SSL settings (sslverify, sslcacert, sslclientkey, sslclientcert)
are properly written to repository configuration files.
...
my $cmp = NCM::Component::spma::yumng->new("spma");

# Mock external dependencies so Configure can run
my $mock = Test::MockModule->new('NCM::Component::spma::yumng');
...
$mock->mock('execute_command', sub {
    ...
    if ($cmd_str =~ /rpm -qa/) {
        return (0, "0:ConsoleKit-4.1-3.el6.x86_64\n0:ncm-spma-1.0-1.noarch\n", "");
    }
    if ($cmd_str =~ /repoquery.*--all/) {
        return (0, "0:ConsoleKit-4.1-3.el6.x86_64\n0:ncm-spma-1.0-1.noarch\n", "");
    }
    return (0, "", "");
});
...
$mock->mock('get_installed_rpms', sub {
    return Set::Scalar->new("0:ConsoleKit-4.1-3.el6.x86_64", "0:ncm-spma-1.0-1.noarch");
});
