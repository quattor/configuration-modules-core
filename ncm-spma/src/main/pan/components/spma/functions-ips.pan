...
declaration template components/spma/functions-ips;
...
function pkg_repl = {
    ...
    ##########################################################################
    # Even though Solaris IPS now uses SPMA, it cannot use pkg_repl() due to
    # the reliance on package_defaults which is a mechanism that is not well
    # suited to IPS and one which will hopefully be replaced when Linux
    # moves to a Yum-based model
    ##########################################################################
