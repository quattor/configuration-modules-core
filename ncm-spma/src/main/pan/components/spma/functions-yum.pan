...
declaration template components/spma/functions-yum;
...
#
# If [version] or [arch] are not defined, their value is upon YUM to
# resolve (latest in repository).
#
...
#######################################################################
#
# Function to make new repository visible to spma upon its invocation.
# It takes exactly one argument - name of the repository for which
# respective repository file exists in repository/ directory hierarchy.
#
#######################################################################

function repo_add = {
        append(nlist("name", ARGV[0]));
};
...
# Function to make new repository visible to spma upon its invocation.
# It takes exactly one argument - name of the repository for which
# respective repository file exists in repository/custom-staging
# directory. The purpose of this function is to specify repositories
# out of ordinary OS stages.
#
#######################################################################


function repo_add_custom_staging = {
        append(nlist("name", "__CUSTOM_STAGING__" + ARGV[0]));
};
