...
=head2 /software/components/filecopy/conditional
A boolean that defines if the restart (if any defined) of the file(s) 
should be made conditional (ie restart if something has changed to 
...
can be either a owner or 'owner:group'.

Also a 
per filename based "conditional" can be given to overwrite 
