...
Remember that the settings for an user means B<the ways to log in as
that user>
...
authoritation information for each user, and C<roles> will contain a
...
=item * /software/components/useraccess/configSerial

This property is an arbitrary string representing a configuration serial number. It is not interpreted in any way by
the component. Its role is only to trig a component run when its value changes. This is necessary when the change is
in a file external to the configuration, referred by a URL.

=item * /software/components/useraccess/{users,roles}/<id>/kerberos4

It is a list of the users who can log in using Kerberos v4
tickets. The contents of this list will be appropiately formatted and
written into ~/.klogin.

=item * /software/components/useraccess/{users,roles}/<id>/kerberos5

It is a list of the users who can log using Kerberos v5 tickets. The
contents of this list will be appropiately formatted and written into
~/.k5login.

=item * /software/components/useraccess/{users,roles}/<id>/ssh_keys_urls
...
=item * /software/components/useraccess/{users,roles}/<id>/ssh_keys
...
=item * /software/components/useraccess/{users,roles}/<id>/acls

It is a list of the ACL-controlled services the user can use. This
only applies to PAM controlled services. SSH is not (not neccessaryly)
controlled by PAM.

B<IMPORTANT NOTE> every service listed just once here will be
converted to use ACLs. The old ncm-access_control behaviour can be
obtained by setting ACLs on system-auth .

=item * /software/components/useraccess/{users,roles}/<id>/roles
...
=item * /software/components/useraccess/<id>/kerberosX/realm : mandatory

Kerberos' realm for authentication (the part behind the @ in
.klogin). For instante, CERN.CH .

=item * /software/components/useraccess/<id>/kerberosX/principal :
mandatory

Principal identity for the user in the Kerberos ticket server.

=item * /software/components/useraccess/<id>/kerberosX/instance :
optional

"Instance" indentity for the user. This is a sub-identity, and it's
optional.

=item * /software/components/useraccess/<id>/kerberosX/host : optional
...
get all the appropiate settings:
...
=head2 One user who can be sued

This example ilustrates how tricky ACLs can get:
...
su foo from any user you may want.

=head2 CERN's current ACLs

CERN's current ACLs can be obtained by setting the system-auth service to use ACLs:
...
And now, the only who can log-in on any service that stacks sytem-auth
are the Simpsons!
...
may still be able to log-in with his public key. You can run C<ncm-ncd
--unconfigure useraccess> to temporarily lock all accuonts, before
