...
# Modifies an existing user using usermod, based on user hash. Has to be before add_user
# IN:  self (for logging), user hash.
# OUT:
##########################################################################
sub modify_user {
##########################################################################
...
##########################################################################
    ...
    # If the home directory is defined, then ensure that the parent
    ...
    if (defined($user->{"homedir"})) {
        ...
        if (! -d $dname) {
            $self->error("can't create $dname; skipping user $username");
            return 1;
        }
        else {
            $self->log("Creating parent directory $dname for user $username");
        }
    }
...
##########################################################################
    ...
    if (exists $prof_hash->{"groups"}){
        ...
        $user_hash->{"groups"}=join(',',@$grouplist);
...
##########################################################################
    ...
    while(my $pw  = &$getUserFunction($username)){
        ...
        my $cammdtoExec = $SYSID." -Gn ". $pw->name;
        my $groups=output($cammdtoExec);
        chomp($groups);
        ...
        if ($? == 0) {
            ...
            $self->warn("Failed to get groups for existing user ". $pw->name.".");
        }
