...
# Return true on success, else false. Don't signal error in this function,
# let's the caller do it.

sub prepare_home {
    my ($self, $homedir) = @_;

    my @spl = split("/", $homedir);
    $homeparent = join("/",  @spl[0 .. (scalar(@spl)-2)]);
    ...
    if ( -d $homeparent ) {
      ...
      $status = makedir($homeparent,0755);
    }
...
##########################################################################
    ...
    if (defined($user->{"homeDir"})) {
        ...
        if ( $user->{"createHome"} ) {
          ...
          $self->log("Created home parent directory $dname for user $username");
        }
    }
...
##########################################################################
    ...
    if ( $user->{"createHome"} ) {
      ...
      $self->log("Created home parent directory $dname for pool account $username");
    }
