...
The I<accounts> component manages the accounts on a machine.
...
=head2 /software/components/accounts/shadowpwd

Boolean indicating whether shadow passwords are being used or not.

=head2 /software/components/accounts/users

An nlist of users to configure on the node.  The key is the account
name (or base name for pool accounts).  At least one field must be
specified.  The available fields are:
...
default. If not given it will result in a locked account.

=item shell

the shell for the user.  Defaults to /bin/bash.

=item uid

the uid value for this account.  If not given it will
default to an unused value.  This is interpreted as the base uid value
for pool accounts (i.e. poolSize > 0).
...
=head2 /software/components/accounts/ldap

If set to true the component will assume that the system is set up
to use LDAP. All changes are made with libuser.
