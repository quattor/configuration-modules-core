...
=item * C<< /software/components/ntpd/active >> : boolean

activates/deactivates the component.

=item * C<< /software/components/ntpd/useserverip >> : resolve and use the time server(s) ip address in the config file(s)

=item * C<< /software/components/ntpd/servers/ >> : list of time servers

=item * C<< /software/components/ntpd/serverlist/ >> : list of C<< { server=hostname, options=nlist() } >>
...
Default : C<< /software/components/ntpd/defaultoptions >> or C<< None >> if not specified
and defaultoptions.
...
=item * C<< /software/components/ntpd/restrictdefault >> : nlist (optional)
...
=item * C<< /software/components/ntpd/authenticate >> : boolean (optional)

Adds string C<< 'authenticate yes' >> to ntp.conf.

Default : None

=item * C<< /software/components/ntpd/broadcastdelay >> : double (optional)

Double value in seconds to set network delay between local and remote servers.

Refer to C<< man ntp_misc >> for more details.

Default : None

=item * C<< /software/components/ntpd/keyfile >> : string (optional)
...
Refer to C<< man ntp_auth >> for more details.

Default : None

=item * C<< /software/components/ntpd/trustedkey >> : list of integers (optional)
...
=item * C<< /software/components/ntpd/requestkey >> : long (optional)
...
=item * C<< /software/components/ntpd/controlkey >> : long (optional)
...
=item * C<< /software/components/ntpd/driftfile >> : string (optional)
...
=item * C<< /software/components/ntpd/includefile >> : string (optional)
...
=item * C<< /software/components/ntpd/logfile >> : string (optional)

Refer to C<< man ntp_misc >> for more details.

Default : None

=item * C<< /software/components/ntpd/logconfig >> : list of strings (optional)
...
=item * C<< /software/components/ntpd/statsdir >> : string (optional)

Refer to C<< man ntp_misc >> for more details.

Default : None

=item * C<< /software/components/ntpd/statistics >> : nlist (optional)
...
=item * C<< /software/components/ntpd/filegen >> : list of nlist (optional)
...
value can be set to ' C<< clockstats | cryptostats | loopstats | peerstats | rawstats | sysstats' >>

=item file : string (optional)

=item type : string (optional)

value can be C<< 'none | pid | day | week | month | year | age' >>
...
=item * C<< /software/components/ntpd/enable >> : nlist (optional)
...
=item * C<< /software/components/ntpd/disable >> : nlist (optional)
...
=item * C<< /software/components/ntpd/tinker >> : nlist (optional)
...
=item * C<< /software/components/ntpd/servicename >> : string (optional)
...
Default : C<< "ntpd" (linux), "svc:/network/ntpd" >> (solaris)

=back

=over

=item * C<< /software/components/ntpd/includelocalhost >> : boolean (optional)
...
=item * C<< /software/components/ntpd/enablelocalhostdebug >> : boolean (optional)

Allows some debugging via ntpdc on localhost but with no modifications

Default : true

=item * C<< /software/components/ntpd/group >> : defined_group (optional)

If the group is set, files are written with C<< root.<group> >> ownership
and C<0640> permission.
