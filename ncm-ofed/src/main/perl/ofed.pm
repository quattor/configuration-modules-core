...
sub opensm
{
    ...
    if ($tree->{config}) {
        $changed += $self->_render(
            $config->getElement($self->prefix().'/opensm/config'),
            $OPENSM_CONF_TT,
            $OPENSM_CONF_FILENAME
        ) || 0;
    }
