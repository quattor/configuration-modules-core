...
##########################################################################
    ...
    ## resources
    $crmpath="$valPath/crm/resources";
    $crm.="##\n## resources\n##\n";
    foreach my $c ( qw(primitive monitor group clone master) ) {
        if ($config->elementExists($crmpath."/".$c)) {
            $val=$config->getValue($crmpath."/".$c);
            $crm.="$c $val\n";
        }
    }
    $crm.="## end resources\n\n";
    ...
    ## constraints
    $crmpath="$valPath/crm/constraints";
    $crm.="##\n## constraints\n##\n";
    ...
    ## attributes
    $crmpath="$valPath/crm/attributes";
    $crm.="##\n## attributes\n##\n";
    ...
    if (LC::Process::execute(['/usr/sbin/crm'],
        ...
        $self->info("CRM applied. (CRM file in $crmloc)");
    } else {
        $self->info("Applying CRM failed. CRM file in $crmloc; output $out");
    }
