...
The I<profile> component creates two scripts (sh and csh flavor, respectively .sh and .csh extension) in
the given configuration directory.  This directory by default is
/etc/profile.d.  These scripts contain environmental variable and path
definitions.

Note that the only guarantee with respect to order is that all the
plain environmental variables will be defined before the paths.
...
A hash containing the environmental variables to define in default script. The environmental variable name is the key and the
value is a string.

=item path

A structure defining (optionally) paths to define in default script. It may contain a prepend, append, and value
elements.  These elements are each a list of strings. 
...
Only the first occurance of a particular path will be kept in the
...
A nlist describing contents of scripts other than the default one. Key is the script base name
...
Value is a nlist that may contains 'env' and 'path' property (as in the default script) plus the properties described
below.
...
This property indicates wether to add a .sh or .csh suffix to the script path. If false, only one flavor must be 
specified.
...
This functions allows to add an environment variable to a script. It returns component profile configuration. There are 2
calling formats :
...
In second form, 'env_list' is a list of nlist. Each nlist must be a pair of environment variable name and value.

=head2 component_profile_add_path()

This functions allows to add a path variable to a script. It returns component profile configuration. The calling format is:
...
=item prepend: this value is prepending to an existing value, if any
