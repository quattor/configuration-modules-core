...
=head2 /software/components/postfix
...
C</etc/postfix/main.cf>. See L<main(5)> for more details.

=item C<master>

A list with all entries for C</etc/postfix/master.cf>. See
L<master(5)> for all the details. For each line, we have to provide:
