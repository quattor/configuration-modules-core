...
=item * If the component finds any issue during hypervisor host configuration then the node is not included within OpenNebula infrastructure (the host is set as disable)
