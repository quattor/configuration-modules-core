...
=item * C<< /software/components/nfs/exports >>
...
the value the export options(e.g. for C<< "nfsclient.example.org(rw)" >>,
key will be C<< escape("nfsclient.example.org") >> and value will be C<< 'rw' >>).  Note that the values in "hosts"
...
=item * C<< /software/components/nfs/mounts >>
